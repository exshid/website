  ...
  return (
    <Base>
      ...
      <div className="px-4 pt-3 md:px-6 lg:p-6 xl:px-20">
        <span className="text-black text-3xl font-semibold uppercase pl-3">
          Latest
        </span>
      </div>
...
export async function getStaticProps() {
  ...
  let news = "News";
  let games = "Games";
  let tvPosts = reversedRweets
    .filter((item) => item.cats.includes(tv))
    .slice(0, 3);
  let moviesPosts = reversedRweets
    .filter((item) => item.cats.includes(movies))
    .slice(0, 5);
  let gamesPosts = reversedRweets
    .filter((item) => item.cats.includes(games))
    .slice(0, 4);
  let newsPosts = reversedRweets
    .filter((item) => item.cats.includes(news))
    .slice(0, 4);
