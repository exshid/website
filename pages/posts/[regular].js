...
function Tweet(props, relatedPosts) {
  ...
  let relatedPostsFiltered = props.relatedPosts.filter((obj) =>
    obj.cats.includes(myCats[0])
  );

  if (relatedPostsFiltered.length < 3) {
    let relatedPostsCompleted = props.relatedPosts.filter(
      (obj) => !relatedPostsFiltered.includes(obj)
    );
    relatedPostsFiltered = relatedPostsFiltered.concat(
      relatedPostsCompleted.slice(0, 3 - relatedPostsFiltered.length)
    );
  }
      ...
      <Base title={`${props.rweetData.title} - ${title} `}>
        ...
        >
                ...
                {myCats.map((item, index) => (
                  <Fragment key={index}>
                    <Link key={Math.random()} href={`/categories/${item}`}>
                      <span className="text-pink-800 hover:underline text-lg uppercase w-max inline-block">
                        {item}
                      </span>
                    </Link>
                    {index !== myCats.length - 1 && (
              ...
              <div className="flex flex-row lg:flex-col">
                ...
                >
                  <span className="text-lg uppercase font-bold text-black hover:underline hover:text-gray-500 inline-block lg:block w-max lg:w-full">
                    {props.rweetData.author}
                  </span>
                </Link>
                <span className="inline-block w-max lg:hidden	mx-2">•</span>
                <span className="text-lg w-max lg:w-full">
                  {formatDate(props.rweetData.date)}
                </span>
              </div>
            </div>
          ...
          <div
            ...
            dangerouslySetInnerHTML={{ __html: props.rweetData.content }}
          />
          {myTags.length > 0 && (
            <div className="flex space-x-4 px-3 pt-3 pb-2 mt-2 w-full lg:w-4/5 items-center border-y">
          ...
          <div className="w-full lg:w-4/5 flex flex-col items-center mb-6">
            <span className="text-3xl text-black pt-4 font-bold">
              Read More
            </span>
                    ...
                    <div className="flex flex-col">
                      ...
                      <div className="pt-8">
                        ...
                        <Link href={`/authors/${post.author}`}>
                          <span className="mt-2 text-gray-700 hover:underline hover:text-black">
                            {post.author}
                          </span>
                        </Link>
                      </div>
...
export async function getStaticPaths() {
  ...
  const rweets = await tweetsCollection
    .find(
      {},
      {
        _id: 1,
...
export async function getStaticProps(context) {
  ...
  const relatedPosts = allPosts
    .filter((post) => post._id.toString() !== tweetId)
